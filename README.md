...
**`deploy.ps1` の処理内容（対話式で設定を入力）:**
1. Node.js ポータブル版のセットアップ（未取得の場合）
2. NSSM（サービス化ツール）のセットアップ（サービス登録時のみ）
3. 依存パッケージインストール（`npm install` + `prisma generate`）
4. `npm run build` でプロダクションビルド
5. デプロイ先ディレクトリの準備（既存サービス停止含む）
6. 必要なファイルをデプロイ先にコピー
7. `.env` ファイルを生成（JWT_SECRET 自動生成）
8. データベース初期化（マイグレーション + 初期データ投入）
9. Windows サービスとして登録・起動（サービス登録時のみ）
10. IIS サイト作成・ARR プロキシ有効化（サービス登録時のみ）
...
> **Note**: IIS サイトの作成は `deploy.ps1` が自動で行います。手動で作成・変更する場合は IIS マネージャー（`inetmgr`）から「サイト」→「Web サイトの追加」で以下を設定してください：
>
> | 項目 | 設定値 |
> |------|--------|
> | サイト名 | `QuoteSystem`（任意） |
...
Node.js サーバーは Windows サービスとして自動管理されます：
- サーバー起動時に自動起動
- クラッシュ時に自動再起動
